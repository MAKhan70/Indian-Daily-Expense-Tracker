...
- Dashboard mini trend and budget ring expose amount readouts on hover, touch and keyboard selection. The enlarged mobile ring centre opens Pie Analytics; coloured segments show amounts underneath without navigation. Keep the trend Analytics link separate and omit the text link below the ring.
- Mobile home navigation includes Monthly Grocery List at the far right next to Budget, while retaining its hamburger menu entry.
- Group the dashboard available-to-spend summary and budget ring inside a rounded, softly bordered surface matching the light-grey Spent and Trend tiles, with theme-aware colours.
