...
- Keep English as the default language and offer all 22 Eighth Schedule Indian languages as account-synced choices. A language change must apply across the complete authenticated app, public authentication and recovery flow, navigation, forms, dialogs, charts, notifications, accessible names, and document metadata. Preserve user-entered names verbatim, persist the choice on the device through sign-out, and apply right-to-left document direction for Urdu, Kashmiri, and Sindhi.
